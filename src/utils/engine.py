...
def test_multimodal(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, loss_fn: torch.nn.Module,
         device: torch.device):
    """Tests a PyTorch model for a single epoch.
...
def train_step_multimodal(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, loss_fn: torch.nn.Module,
               optimizer: torch.optim.Optimizer, device: torch.device) -> Tuple[float, float]:
    """Trains a PyTorch model for a single epoch.
...
def train(model: torch.nn.Module, train_dataloader: torch.utils.data.DataLoader,
    ...
    for epoch in tqdm(range(epochs), colour="BLUE"):
        ...
        train_step_fn = train_step if task is None else train_step_multimodal
        test_fn = test if task is None else test_multimodal
